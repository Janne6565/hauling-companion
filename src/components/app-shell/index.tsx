...
export function AppShell({ phase, onPhaseClick, children, unlockedUpTo }: AppShellProps) {
  const phaseIdx = PHASES.findIndex((p) => p.id === phase);
  const unlockedIdx = unlockedUpTo ? PHASES.findIndex((p) => p.id === unlockedUpTo) : phaseIdx;
      ...
      >
        <div className="flex items-center gap-2.5 font-mono text-xs tracking-widest uppercase">
          <div
            ...
            style={{ border: '1.5px solid var(--primary)', background: 'oklch(0.22 0.02 200)' }}
          >
            <div
                ...
                >
                  <span
                    className={cn(
                      ...
                      isDone && "border-muted-foreground bg-muted-foreground text-primary-foreground",
